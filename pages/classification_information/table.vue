            ...
            <view class="">
              ...
              <uni-forms :modelValue="query">
                  <uni-forms-item label="分类编号" name="classification_number">
                      <uni-easyinput type="text" v-model="query.classification_number" placeholder="分类编号" />
                    </uni-forms-item>
				...
				<view v-for="(o,i) in list" class="row">
                            ...
                            <span>分类类别</span>
								...
								<text>
									{{ o["classification_categories"] }}
													...
													<view v-if="0 && $check_field('get','classification_remarks')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>分类备注</span>
                          </view>
                  ...
                  <view class="col">
					...
					<navigator  :url="'/pages/classification_information/view?' + field + '=' + o[field]"
	                    ...
	                    <a @click="goto_pay(o)" size="mini"  v-if="o.pay_state==='未支付' && $check_pay('/classification_information/table')" class='btn btn-primary'>支付</a>
	            					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
          ...
          goto_pay(v) {
            //打开选择支付方式
            this.pay_obj.id = v.classification_information_id
            this.model=true;
          },
          choosepayway(idx){
            switch(idx){
          ...
          submit_pay(){
            let _this = this;
            let url = this.$toUrl(this.query, "~/api/classification_information/set?classification_information_id="+this.pay_obj.id);
            let param = {"pay_state":"已支付","pay_type":this.pay_obj.payActiveName}
            this.$post(url,param,(res)=>{
              if(res.result){
                _this.$toast("支付成功");
                setTimeout(()=>{
                  _this.$nav("/pages/classification_information/table");
                }, 700);
              }else if(res.error){

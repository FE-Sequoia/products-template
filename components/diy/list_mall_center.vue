	...
	<view class="list_mall_center list_com--new" style="background-color: #fff;">
		...
		<view class="mall_center_block box_wrap" >
			<navigator class="item_mall_center box_style" v-for="(o, i) in list"  :key="i" :url="'/pages/mall_center/details?mall_center_id=' + o['mall_center_id']">
					<view class="view" v-if="0 && $check_index_field('get','mall_seller','/mall_center/list')">
					<view class="title" v-if="true">
						<span>商城卖家</span>
					</view>
							<view class="diy_field time" >
						<span>{{ get_user_mall_seller(o['mall_seller']) }}</span>
					</view>
						</view>
						<view class="view" v-if="0 && $check_index_field('get','merchant_name','/mall_center/list')">
					<view class="title" v-if="true">
						<span>商家姓名</span>
					</view>
						...
						<view class="view" v-if="0 && $check_index_field('get','production_date','/mall_center/list')">
					<view class="title" v-if="true">
						<span>生产日期</span>
					</view>
							<view class="diy_field time" >
						...
						<view class="view" v-if="0 && $check_index_field('get','shelf_life','/mall_center/list')">
					<view class="title" v-if="true">
						<span>保质期</span>
					</view>
							<view class="diy_field time" >
						<span>{{ $toTime(o["shelf_life"] ,"yyyy-MM-dd hh:mm:ss") }}</span>
					</view>
						</view>
						<view class="view" v-if="0 && $check_index_field('get','label','/mall_center/list')">
					<view class="title" v-if="true">
						<span>标签</span>
					</view>
							<view class="diy_field text" >

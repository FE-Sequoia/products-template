	...
	<div class="diy_details_box diy_div_mall_center">
		<div class="warp diy_warp">
			...
			<div class="details_btn_wrap">
																				<button class="btn btn-primary details_btn" v-if="$check_action('/mall_center/details','set')" @click="change_collect()" ><span>收藏</span></button>
											</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			async get_list_user_mall_seller() {
				...
				var obj = null;
					if (name == 'mall_seller'){
					obj = this.list_user_mall_seller.getObj({"user_id":id});
				}
									var ret = "";
				if(obj){

			...
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
																				<uni-search-bar placeholder="搜索生产日期" @confirm="search_" @cancel="search_cancel('production_date')" cancelText="取消"
											@input="input($event, 'production_date')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																				<uni-search-bar placeholder="搜索标签" @confirm="search_" @cancel="search_cancel('label')" cancelText="取消"
											@input="input($event, 'label')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
													<!-- /搜索栏 -->
						</view>
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
										<uni-section title="商城卖家" type="line">
							<uni-data-select
									@change="change_mall_seller"
			...
			return {
				url_get_list: "~/api/mall_center/get_list?like=0",
					...
					list_user_mall_seller: [],
							list_mall_seller: [{value:"全部",text:"全部"}],
																list_orderby: [
									{
					name: '发布时间',
					direction: "",
	...
	get_list_after(json ,func){
		...
		if (func) {
			func(json);
				...
				// 改变分类标签
	change_mall_seller(val) {
		if (val === "全部" || val === "") {
			this.query.mall_seller = "";
		}else{
			this.query.mall_seller = val
		}
		this.search_();
	},

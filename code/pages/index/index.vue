<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="通知公告推荐"
      url="/pages/notification_announcement/list"
      :list="list_notification_announcement"
      v-if="$check_action('/notification_announcement/list', 'get')"
    >
	  <list_notification_announcement :list="list_notification_announcement"></list_notification_announcement>
...
export default {
  ...
  methods: {
      ...
      this.sendValue = "";
		...
		*/
		get_customized_products() {
			let url = getApp().globalData.host + "/api/customized_products/get_list?";
				...
				"&like=0";
				url = url_get_list;
													url = getApp().globalData.host + "/api/customized_products/get_hits_list?";
			                        let param = {
					...
					this.list_customized_products = json.result.list;
						...
						this.get_praise(this.list_customized_products ,"customized_products" ,"44885");
									}
			});
		},
    				/**
		...
		*/
		get_notification_announcement() {
			let url = getApp().globalData.host + "/api/notification_announcement/get_list?";
									                        let param = {
                                    }
            			this.$get(url, param, (json) => {
					if (json.result && json.result.list) {
					console.log("通知公告" ,json.result.list);
					this.list_notification_announcement = json.result.list;
											this.list_notification_announcement.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_notification_announcement ,"notification_announcement" ,"83301");
									}
			});
		},
    ...
    get_menu() {
      ...
      this.$get(
		...
		this.list_menu.push({mod_name:"商城中心",path:"/goods/list"})
	          this.list_menu.unshift({ mod_name: "在线考试", path: "/exam/list" });
          this.list_menu.unshift({ mod_name: "自动组卷", path: "/exam_subject/list" });
    ...
    get_goods() {
        ...
        let query = { page: 1, size: 12 }
                                            url = "/api/mall_center/get_tag_list?"
        if (this.$store.state.user.user_group!="管理员"){
            query.user_id = this.$store.state.user.user_id
        }
                                                                  this.$get(getApp().globalData.host + url,query , (json) => {
        if (json.result && json.result.list) {
  ...
  onShow() {
						...
						this.get_notification_announcement();
		    this.get_menu();
    this.get_slides();
    this.get_article();
    this.get_goods();

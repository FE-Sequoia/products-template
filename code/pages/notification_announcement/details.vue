<template>
	<view class="page_notification_announcement diy_detail diy_detail--new" id="notification_announcement_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','announcement_title')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','announcement_type')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>公告类型</span>
							</view>
								<view class="diy_field diy_text">
								<span>
									{{ obj["announcement_type"] }}
								</span>
							</view>
							</view>
					</view>
					<view v-if="$check_field('get','announcement_content')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>公告内容</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=notification_announcement&source_field=notification_announcement_id&source_id=' + obj['notification_announcement_id']">
							评论
						</navigator>
	...
	export default {
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					...
					source_field: "notification_announcement_id",
					source_id: obj["notification_announcement_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "notification_announcement",
					source_field: "notification_announcement_id",
					source_id: this.obj["notification_announcement_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/notification_announcement/set?notification_announcement_id=' + _this.obj["notification_announcement_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/notification_announcement/set?notification_announcement_id=' + obj["notification_announcement_id"], {
					hits
				}, (res) => {
					if(res.result){

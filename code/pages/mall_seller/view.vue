        ...
        <view>
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','seller_id') || ($check_field('add','seller_id') || $check_field('set','seller_id'))" label="卖家编号" name="seller_id">
                            <uni-easyinput type="text" v-model="form['seller_id']" v-if="user_group === '管理员' || (form['mall_seller_id'] && $check_field('set','seller_id')) || (!form['mall_seller_id'] && $check_field('add','seller_id'))" :disabled="disabledObj['seller_id_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','seller_id')">
                  {{ form['seller_id'] }}
                </text>
                          </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','seller_name') || ($check_field('add','seller_name') || $check_field('set','seller_name'))" label="卖家姓名" name="seller_name">
                            <uni-easyinput type="text" v-model="form['seller_name']" v-if="user_group === '管理员' || (form['mall_seller_id'] && $check_field('set','seller_name')) || (!form['mall_seller_id'] && $check_field('add','seller_name'))" :disabled="disabledObj['seller_name_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','seller_name')">
                  {{ form['seller_name'] }}
                </text>
                          </uni-forms-item>
            ...
            <view class="form_button" v-if="$check_action('/mall_seller/view','set') || ($check_action('/mall_seller/view','add') || $check_option('/mall_seller/table','examine'))">
...
import mixin from "@/libs/mixins/page.js";
  ...
  data() {
    return {
      ...
      group_table: "mall_seller",
      is_password: true,
          field: "mall_seller_id",
      url_add: "~/api/mall_seller/add?",
      url_set: "~/api/mall_seller/set?",
      url_get_obj: "~/api/mall_seller/get_obj?",
      url_upload: "~/api/mall_seller/upload?"
	  ,seller_id: null
	  ,seller_name: null
      ,query: {
        "mall_seller_id": 0,
  ...
  methods: {
	...
	,uploadFile_(type) {
		...
		,successChoose(filePath, type) {
			uni.uploadFile({
				url: getApp().globalData.host + '/api/mall_seller/upload', //仅为示例，非真实的接口地址
				filePath,
				name: 'file',
	...
	,close_(type) {
					if (type == 'seller_id') this['seller_id'] = this.form['seller_id'] = ""
					if (type == 'seller_name') this['seller_name'] = this.form['seller_name'] = ""
			}
	,submit_() {
					if (this['seller_id'] !== null) this.form['seller_id'] = this['seller_id']
					if (this['seller_name'] !== null) this.form['seller_name'] = this['seller_name']
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/mall_seller/upload?'),
    ...
    submit_sub(form_sub){
      if(form_sub["mall_seller_id"]){
        // 提交事件
        this.$post("~/api/" + this.group_table + "/set?mall_seller_id=" + form_sub["mall_seller_id"],form_sub,(res)=>{
    ...
    is_view(){
      ...
      if(!bl){
        bl = this.$check_action('/mall_seller/table','add');
        console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
      }
      if(!bl){
        bl = this.$check_action('/mall_seller/table','set');
        console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
      }
      if(!bl){
        bl = this.$check_action('/mall_seller/view','add');
        console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
      }
      if(!bl){
        bl = this.$check_action('/mall_seller/view','set');
        console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
      }
      if(!bl){
        bl = this.$check_action('/mall_seller/view','get');

  ...
  <view class="page_order" id="order_details">
    ...
    <view class="">
      <template v-if="$check_action('/order/details', 'get')">
        ...
        <view class="sum_price flex_sbc" v-if="state === '待付款'">
          <picker v-if="use_coupon_list.length>0" style="margin-left: 5px;width: 100%;" mode="selector"
                  :value="selected_coupon_index" :range="use_coupon_list" range-key="coupon_name"
                  @change="bindPickerChange">
            <view style="line-height: 2.3rem;" class="uni-input">
              选择优惠券：{{use_coupon_list[selected_coupon_index].coupon_name}}
            </view>
          </picker>
          <view v-if="use_coupon_list.length <= 0">无可使用的优惠券</view>
        </view>
        <view v-if="vip_discount>0" class="sum_price flex_sbc">
          <text>会员折扣</text>
  ...
  methods: {
    ...
    async get_list_after(json) {
      ...
      for (let i = 0; i < this.coupon_list.length; i++) {
        ...
        for (let j = 0; j < result.length; j++) {
          if (result[j].merchant_id === this.coupon_list[i].coupon_user_id || this.coupon_list[i]
                  .coupon_user_auth === "管理员") {
      ...
      for (let i = 0; i < this.use_coupon_list.length; i++) {
        if (this.use_coupon_list[i].coupon_id){
    ...
    goto_pay() {
      ...
      if (this.list.length>0){
        ...
        this.$get("~/api/order/alipay?goods_name=" + goods_name + "&pay_money=" + this.sum_price, {}, (json) => {
          if (json.result) {
      ...
      this.$post(
        ...
        (res) => {
          if (res.result) {
            ...
            _this.$toast("支付成功");
            const id = this.use_coupon_list[this.selected_coupon_index].id
                if (id) {
    ...
    bindPickerChange: function(e) {
      this.selected_coupon_index = e.detail.value
    },
  },
  computed: {
    sum_price() {
      ...
      if (this.use_coupon_list && this.use_coupon_list.length>0){
        const coupon_id = this.use_coupon_list[this.selected_coupon_index].coupon_id

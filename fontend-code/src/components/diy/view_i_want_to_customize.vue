	...
	<el-main class="bg edit_wrap">
		...
		<el-row class="row_ce">
				...
				<el-form-item label="订单号" prop="order_number">
							  ...
							  v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','order_number')) || (!form['i_want_to_customize_id'] && $check_field('add','order_number'))" :disabled="true"></el-input>
				...
				<el-form-item label="商品编号" prop="product_number">
							  ...
							  v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','product_number')) || (!form['i_want_to_customize_id'] && $check_field('add','product_number'))" :disabled="disabledObj['product_number_isDisabled']"></el-input>
				...
				<el-form-item label="封面" prop="cover">
						...
						:show-file-list="false" v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','cover')) || (!form['i_want_to_customize_id'] && $check_field('add','cover'))">
				...
				<el-form-item label="金额" prop="money">
												<el-input id="money" v-model="form['money']" placeholder="请输入金额"
							  v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','money')) || (!form['i_want_to_customize_id'] && $check_field('add','money'))" :disabled="disabledObj['money_isDisabled']"></el-input>
				...
				<el-form-item label="购买用户" prop="purchasing_users">
							...
							<!--v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','purchasing_users')) || (!form['i_want_to_customize_id'] && $check_field('add','purchasing_users'))" :disabled="disabledObj['purchasing_users_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','purchasing_users')">{{form['purchasing_users']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','purchasing_users')) || (!form['i_want_to_customize_id'] && $check_field('add','purchasing_users'))" id="purchasing_users" v-model="form['purchasing_users']" :disabled="disabledObj['purchasing_users_isDisabled']">
				...
				<el-form-item label="总金额" prop="total_amount">
					...
					<div v-else-if="$check_field('get','total_amount')">{{form['total_amount']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','customized_time') || $check_field('add','customized_time') || $check_field('set','customized_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="定制时间" prop="customized_time">
								<el-date-picker :disabled="disabledObj['customized_time_isDisabled']" v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','customized_time')) || (!form['i_want_to_customize_id'] && $check_field('add','customized_time'))" id="customized_time"
						v-model="form['customized_time']" type="datetime" placeholder="选择日期时间">
					</el-date-picker>
					<div v-else-if="$check_field('get','customized_time')">{{form['customized_time']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','requirement_document') || $check_field('add','requirement_document') || $check_field('set','requirement_document')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="需求文件" prop="requirement_document">
												<div v-if="disabledObj['requirement_document_isDisabled']">
						<div v-if="$check_field('get','requirement_document')">
							<el-button type="primary" @click="$download($fullUrl(form['requirement_document']),form['requirement_document'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','requirement_document')) || (!form['i_want_to_customize_id'] && $check_field('add','requirement_document'))" class="upload-demo" drag
								   action="" style="max-width: 300px;width: 100%;" :http-request="upload_requirement_document" :limit="1" accept="">
							<i class="el-icon-upload"></i>
							<div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
						</el-upload>
						<div v-else-if="$check_field('get','requirement_document')">
							<el-button type="primary" @click="$download($fullUrl(form['requirement_document']),form['requirement_document'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
				...
				<el-form-item label="备注" prop="remarks">
						...
						v-if="user_group === '管理员' || (form['i_want_to_customize_id'] && $check_field('set','remarks')) || (!form['i_want_to_customize_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/i_want_to_customize/view','set') || $check_action('/i_want_to_customize/view','add') || $check_option('/i_want_to_customize/table','examine')">
        ...
        if (_this.$store.state.user.user_group==='管理员'){
          ...
          _this.$get("~/api/customized_products/get_obj?",
			...
			async get_group_user_purchasing_users() {
							this.form["purchasing_users"] = this.$store.state.user.user_id;
			 ...
			 */
			upload_requirement_document(param){
						this.uploadFile(param.file, "requirement_document");
					},
						...
						Object.keys(form).forEach(dbKey => {
							...
							if(key === dbKey){
								this.disabledObj[key+'_isDisabled'] = true;
																			        ...
																			        if (this.form["customized_time"] && this.form["customized_time"].indexOf("-")===-1){
			...
			get_obj_after(json, func){
																																									...
																																									if(json.result.obj["customized_time"]=="0000-00-00 00:00:00"){
				  json.result.obj["customized_time"] = null;
				}
				if(parseInt(json.result.obj["customized_time"]) > 9999){
					json.result.obj["customized_time"] = this.$toTime(parseInt(json.result.obj["customized_time"]),"yyyy-MM-dd hh:mm:ss")
				}

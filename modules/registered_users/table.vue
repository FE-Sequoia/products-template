		...
		<el-form label-position="right" :model="query" class="form p_4" label-width="120">
			<el-row class="rows row1">
					...
					<el-form-item label="分类编号">
									<el-select v-model="query.classification_number">
					...
					<el-form-item label="分类类别">
									<el-input v-model="query.classification_categories"></el-input>
								</el-form-item>
				</el-col>
									<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="user_group == '管理员' || $check_action('/registered_users/table','add') || $check_action('/registered_users/view','add')" @click="$router.push('./view?')">添加</el-button>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="username" label="用户名" width="120">
				...
				<el-table-column prop="vip_level" label="会员等级" sortable min-width="120"></el-table-column>
				...
				v-if="user_group == '管理员' || $check_field('get','age')" min-width="200">
					...
					<el-table-column prop="id_photo" @sort-change="$sortChange" label="证件照"
				v-if="user_group == '管理员' || $check_field('get','id_photo')" min-width="200">
						<template slot-scope="scope">
					<el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['id_photo'])"
						:preview-src-list="[$fullUrl(scope.row['id_photo'])]">
				...
				v-if="user_group == '管理员' || $check_field('get','classification_categories')" min-width="200">
					...
					<el-table-column prop="label" @sort-change="$sortChange" label="标签"
				v-if="user_group == '管理员' || $check_field('get','label')" min-width="200">
					</el-table-column>
					<el-table-column prop="id_card" @sort-change="$sortChange" label="身份证"
				v-if="user_group == '管理员' || $check_field('get','id_card')" min-width="200">
					</el-table-column>
					<el-table-column prop="mobile_phone" @sort-change="$sortChange" label="手机"
				v-if="user_group == '管理员' || $check_field('get','mobile_phone')" min-width="200">
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="user_group == '管理员' || $check_action('/registered_users/table','set') || $check_action('/registered_users/view','set') || $check_action('/registered_users/view','get') || $check_action('//table','add') || $check_action('//view','add')" >
					...
					_this.$get("~/api/user/get_obj?", {
						...
						if (json.result.obj){
							...
							_this.$set(_this.list[i],'nickname',json.result.obj.nickname);
								_this.$delete(_this.list[i],'vip_level');
							_this.$set(_this.list[i],'vip_level',json.result.obj.vip_level);
							}
					});
				}
			},

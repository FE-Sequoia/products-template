		...
		methods: {
          ...
          this.form["production_date"] = this.$toTime(parseInt(this.form["production_date"]),"yyyy-MM-dd")
			...
			get_obj_after(json, func){
														if(this.form["production_date"]=="0000-00-00"){
				  this.form["production_date"] = null;
				}
				if(parseInt(this.form["production_date"]) > 9999){
					this.form["production_date"] = this.$toTime(parseInt(this.form["production_date"]),"yyyy-MM-dd")
				}
								if(this.form["shelf_life"]=="0000-00-00 00:00:00"){
				  this.form["shelf_life"] = null;
				}
				if(parseInt(this.form["shelf_life"]) > 9999){
					this.form["shelf_life"] = this.$toTime(parseInt(this.form["shelf_life"]),"yyyy-MM-dd hh:mm:ss")
				}
                ...
                if (add_flag==='set'){
                    ...
                    this.$post("~/api/goods/set?source_table=mall_center&source_field=" + this.field + "&source_id=" + source_id,this.goods_form,(res)=>{
                        ...
                        if(res.result){
							...
							this.$router.push("/mall_center/table");
                        }else if(res.error){
                            console.error(res.error);
                            this.$toast(res.error.message,"error");
                        }
